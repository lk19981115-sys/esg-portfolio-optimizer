...
export default function DashboardClient({ baseData }: { baseData: ScoredData[] }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#0B1120] font-sans text-slate-200">
      {/* Sidebar */}
      <aside className="w-72 bg-[#111827]/80 backdrop-blur-xl border-r border-slate-800/50 shadow-[4px_0_24px_rgba(0,0,0,0.2)] p-8 flex flex-col z-10 relative">
        <div className="flex items-center gap-2 mb-10">
          ...
          <h1 className="text-lg font-bold text-slate-100 leading-tight">{t.app_title}</h1>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="mt-6 p-4 bg-slate-800/50 rounded-xl border border-slate-700/50 backdrop-blur-sm">
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-3">{t.current_allocation}</p>
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm text-slate-400">🌱 {t.esg}</span>
              <span className="text-sm font-bold text-slate-100">{esgWeight}%</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-400">💰 {t.value}</span>
              <span className="text-sm font-bold text-slate-100">{valueWeight}%</span>
            </div>
          </div>
          ...
          <div className="mt-10">
            ...
            <select 
              ...
              className="w-full p-3 bg-slate-800/80 border border-slate-700/50 rounded-lg text-base font-medium text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/50 shadow-sm cursor-pointer backdrop-blur-md"
            >
      ...
      <main className="flex-1 p-10 h-screen overflow-y-auto relative">
        ...
        >
          <motion.header variants={itemVariants} className="mb-10 flex justify-between items-start">
            <div>
              <h2 className="text-2xl font-bold text-slate-100">{t.overview_title}</h2>
              <p className="text-sm text-slate-400 mt-1">{t.overview_subtitle}</p>
            </div>
            
            {/* Language Selector */}
            <div className="flex items-center gap-2 bg-slate-800/80 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-700/50 shadow-sm">
          ...
          {/* KPI Cards */}
          <motion.div variants={itemVariants} className="grid grid-cols-5 gap-4 mb-10">
        ...
        <motion.div 
          ...
          className="bg-[#111827]/80 backdrop-blur-xl p-6 rounded-2xl border border-slate-800/50 shadow-sm mb-10"
        >
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-4">
              <button 
                onClick={() => setActiveChartTab('cumulative')}
                className={`flex items-center gap-2 text-sm font-bold px-4 py-2 rounded-lg transition-colors ${activeChartTab === 'cumulative' ? 'bg-blue-600/20 text-blue-400' : 'text-slate-400 hover:bg-slate-800'}`}
              >
                <BarChart3 className="w-4 h-4" /> {t.chart_tab_cumulative}
              </button>
              <button 
                onClick={() => setActiveChartTab('scatter')}
                className={`flex items-center gap-2 text-sm font-bold px-4 py-2 rounded-lg transition-colors ${activeChartTab === 'scatter' ? 'bg-blue-600/20 text-blue-400' : 'text-slate-400 hover:bg-slate-800'}`}
              >
            ...
            {activeChartTab === 'cumulative' && (
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-sm font-medium text-slate-400">{t.show_benchmark}</span>
              </label>
            )}
          </div>
        ...
        >
          <div className="p-6 border-b border-slate-800/50">
            <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
              <Target className="w-6 h-6 text-blue-500" /> {t.top_holdings.replace('{year}', selectedYear.toString())}
            </h3>
          </div>
              ...
              <thead>
                <tr className="bg-slate-800/30">
                  {[t.col_ticker, t.col_name, t.col_pe, t.col_esg, t.col_score, t.col_mktcap, t.col_weight].map((h, i) => (
                    <th key={h} className={`py-4 px-6 text-[13px] font-bold text-slate-400 uppercase tracking-wider ${i > 1 ? 'text-right' : ''}`}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-800/50">
                {displayHoldings.map((row) => {
                  ...
                  <tr key={row.Ticker} className="hover:bg-slate-800/50 transition-colors">
                    <td className="py-4 px-6 text-base font-semibold text-slate-100">{row.Ticker}</td>
                    <td className="py-4 px-6 text-base text-slate-300">{row['firm name']}</td>
                    
                    {/* PE Heatmap (Lower is better, so lower PE = greener, higher = redder) */}
                    <td className="py-4 px-6 text-base text-slate-300 text-right font-mono relative">
                      <div className="absolute inset-y-2 right-6 bg-emerald-500/20 rounded-md" style={{ width: `${pePercent}%`, zIndex: 0 }}></div>
                      <span className="relative z-10">{row.pe.toFixed(2)}</span>
                    </td>
                    
                    {/* ESG Heatmap (Higher is better) */}
                    <td className="py-4 px-6 text-base text-slate-300 text-right font-mono relative">
                      <div className="absolute inset-y-2 right-6 bg-emerald-500/20 rounded-md" style={{ width: `${esgPercent}%`, zIndex: 0 }}></div>
                      <span className="relative z-10">{row.adj_esg.toFixed(2)}</span>
                    </td>
                    
                    {/* Combined Score Heatmap (Stretched 0.8-1.0) */}
                    <td className="py-4 px-6 text-base text-slate-300 text-right font-mono relative">
                      <div className="absolute inset-y-2 right-6 bg-blue-500/20 rounded-md" style={{ width: `${scorePercent}%`, zIndex: 0 }}></div>
                      <span className="relative z-10">{row.Combined_Score.toFixed(2)}</span>
                    </td>
                    
                    <td className="py-4 px-6 text-base text-slate-300 text-right font-mono">{(row.mktcap / 1000).toFixed(1)}</td>
                    
                    {/* Weight Data Bar (Scaled down) */}
                    <td className="py-4 px-6 text-base font-bold text-slate-100 text-right font-mono relative">
                      <div className="absolute inset-y-2 right-6 bg-indigo-500/30 rounded-md" style={{ width: `${weightPercent}%`, minWidth: '4px', zIndex: 0 }}></div>
